...
namespace ConstructionLine.CodingChallenge.Tests
{
    public class SearchEngineTestsBase
    {
        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            var defaultColorOptions = searchOptions.Colors.Any() ? searchOptions.Colors : Color.All;
            var defaultSizeOptions = searchOptions.Sizes.Any() ? searchOptions.Sizes : Size.All;
            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts
                    .Count(c => c.Size.Id == size.Id
                                && defaultSizeOptions.Select(sos => sos.Id).Contains(c.Size.Id)
                                && defaultColorOptions.Select(dco => dco.Id).Contains(c.Color.Id));
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts
                    .Count(c => c.Color.Id == color.Id
                                && defaultColorOptions.Select(soc => soc.Id).Contains(c.Color.Id)
                                && defaultSizeOptions.Select(dso => dso.Id).Contains(c.Size.Id));
